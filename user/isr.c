...
// **************************** PIT中断函数 ****************************
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)   //10ms一次编码器测速中断
{
...
IFX_INTERRUPT(cc60_pit_ch1_isr, 0, CCU6_0_CH1_ISR_PRIORITY) //z轴角度计算（去除零飘）
{
    interrupt_global_enable(0);                     // 开启中断嵌套
    ...
    // 更新滑动均值
    if (fabsf(raw_z) < GYRO_IDLE_THRESHOLD)  //如果转动角度 小于1° 则算零飘情况 消去零飘
    {
