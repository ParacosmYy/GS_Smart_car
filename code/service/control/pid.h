 ...
 * pid.h
 *
 * PID 算法接口声明：位置式/增量式 PID 结构体、初始化、计算函数，
 * 以及舵机/电机专用控制函数。
 *
...
float servo_pid_contorl(PosPID_t *steer_pid , float steer_target , float steer_feedback);   ///< 舵机PID控制（位置式，限幅±63）
float motor_pid_control(IncPID_t *motor_pid, float speed_target, float speed_feedback);     ///< 电机PID控制（增量式，限幅±20）
