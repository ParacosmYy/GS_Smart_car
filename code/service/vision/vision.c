 ...
 * Frame geometry comes from PAL. The internal compressed frame is currently
...
#define COL 120          // 原始图像行数（保留备用，实际用 PAL_CAM_H）
#define ROW 188          // 原始图像列数（保留备用，实际用 PAL_CAM_W）

#define FILTER_LEN 20    // 滤波相关参数（保留备用）
#define WHITE_COL 93     // 压缩图像最右列索引（94 列，索引 0-93）

////////////////////大津法二值化//////////////////////
static uint8_t image_threshold = 120;  // 二值化阈值（初值，运行时被 OTSU 覆盖）

// 二值化后的图像缓冲（0=黑，255=白），当前按 PAL 帧尺寸静态分配
static uint8_t mt9v03x_image_bandw[PAL_CAM_H][PAL_CAM_W] = {{0}};
// 二值化后压缩图像，后续算法均基于此
static uint8_t mt9v03x_image_bandw_zip[VISION_ZIP_IMAGE_H][VISION_ZIP_IMAGE_W] = {{0}};


// 边线与中线数据，按行存储（索引 0=图像顶部，59=图像底部）
static uint8_t left_line_list[VISION_ZIP_IMAGE_H] = {0} ; // 每行的左边界列坐标
static uint8_t right_line_list[VISION_ZIP_IMAGE_H] = {0}; // 每行的右边界列坐标
static uint8_t mid_line_list[VISION_ZIP_IMAGE_H] = {0};   // 每行的中线列坐标（左右边界均值）

static uint8_t mid_line = 0;                  // 最终加权平均的中线位置（单值，所有行汇总）
static uint8_t image_mid = IMAGE_MID_COL;     // 图像中心列（94/2 - 1 = 46，这里取 47 作为参考中线）
static int16_t calculate_error = 0;           // 中线偏差 = mid_line - image_mid，作为 PID 控制器输入

static uint8_t lost_count = 0;          // 连续丢线计数，超过阈值时停止控制输出
...
static const uint8_t mid_weight_list[60] = {
    // 行 0-11: 远端低权重 (1)
 ...
 *
 * 处理流水线（顺序执行）：
 *   1. OTSU 计算自适应阈值（基于 PAL 原始灰度图）
 *   2. 按阈值二值化（写入 mt9v03x_image_bandw）
 *   3. 图像压缩（写入 mt9v03x_image_bandw_zip）
 *   4. 二值图像滤波（去孤立噪点）
...
{
    pal_cam_frame_desc_t frame_desc = {0};

    // 步骤 1：OTSU 自适应阈值，图像尺寸由 PAL 帧描述提供
    pal_cam_get_frame_desc(&frame_desc);
    if ((frame_desc.width == 0U) || (frame_desc.height == 0U))
    {
        frame_desc.width = PAL_CAM_W;
        frame_desc.height = PAL_CAM_H;
    }

    image_threshold = otsu(pal_cam_data(), frame_desc.width, frame_desc.height);
    // 步骤 2：按阈值二值化
...
void Vision_GetControlSnapshot(vision_control_snapshot_t *p_snapshot)
{
    ...
    p_snapshot->calculate_error = calculate_error;
    p_snapshot->lost_count = lost_count;
    p_snapshot->mid_line = mid_line;
    p_snapshot->image_mid = image_mid;
    p_snapshot->image_threshold = image_threshold;
}
...
void Vision_GetDebugSnapshot(vision_debug_snapshot_t *p_snapshot)
{
    ...
    p_snapshot->p_binary_zip = mt9v03x_image_bandw_zip;
    p_snapshot->p_left_line = left_line_list;
    p_snapshot->p_right_line = right_line_list;
    p_snapshot->p_mid_line = mid_line_list;
