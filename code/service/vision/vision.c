...
static void set_image_grayscale_to_binary(uint8_t value) //灰度转二值化
{
    ...
    for(uint8_t i = 0 ; i < VISION_RAW_H ; i++)
    {
        for(uint8_t j = 0 ; j < VISION_RAW_W ; j++)
        {
            ...
            if(temp < value)
            {
                s_vision_ctx.binary[i][j] = 0 ; //黑（赛道）
            }
            else
            {
                s_vision_ctx.binary[i][j] = 255 ;//白（背景）
            }
        }
    }
}
...
static void Bin_Image_Filter (void)//过滤噪点 （黑四周有2白 -》 白 ）
{
    ...
    for (nr = 1; nr < VISION_ZIP_IMAGE_H - 1; nr++)
    {
        for (nc = 1; nc < VISION_ZIP_IMAGE_W - 1; nc = nc + 1)
        {
            ...
            if ((s_vision_ctx.binary_zip[nr][nc] == 0) //如果当前色块为黑色
                && (s_vision_ctx.binary_zip[nr - 1][nc] + s_vision_ctx.binary_zip[nr + 1][nc] + s_vision_ctx.binary_zip[nr][nc + 1] + s_vision_ctx.binary_zip[nr][nc - 1] > 2 * 255))
            {
                s_vision_ctx.binary_zip[nr][nc] = 255;
            }
            // 当前为白 且 上下左右白色像素 < 2 个 → 改为黑
            else if ((s_vision_ctx.binary_zip[nr][nc] == 255)//如果当前色块为白色
                && (s_vision_ctx.binary_zip[nr - 1][nc] + s_vision_ctx.binary_zip[nr + 1][nc] + s_vision_ctx.binary_zip[nr][nc + 1] + s_vision_ctx.binary_zip[nr][nc - 1] < 2 * 255))
            {
                s_vision_ctx.binary_zip[nr][nc] = 0;
            }
        }
    }
}
 ...
 */
static uint8_t otsu(uint8_t *image, uint16_t width, uint16_t height)
{
    ...
    for (i = 0; i < GrayScale; i++)
    {
        ...
        else if ((varBetween >= maxVar) && (found_max != 0))
        {
            last_max = i;
        }
    }
...
static void find_mid_line(void)
{
    ...
    for (int8_t row = VISION_ZIP_IMAGE_H - 1; row >= 0; --row)
    {
        ...
        uint8_t left = 0xFF;
        // 从center向左搜索左边界：寻找从白(255)到黑(0)的过渡点
        // 这表示线条的左边缘（假设线条是黑色的，背景是白色的）
        for (int col = center; col > 0; --col)
        {
            // 检查当前像素为黑，左侧像素为白（左边界条件）
            if ((s_vision_ctx.binary_zip[row][col] == 0) &&
                (s_vision_ctx.binary_zip[row][col - 1] == 255))
            {
        ...
        uint8_t right = 0xFF;
        // 从center向右搜索右边界：寻找从黑(0)到白(255)的过渡点
        // 这表示线条的右边缘
...
/**
 * @brief 加权中线计算与丢线检测
 *        对 s_vision_ctx.mid_line_per_row[] 按行权重 mid_weight_list[] 做加权平均，得到单值 s_vision_ctx.mid_line
 *        逐行比较相邻行中线差，若 >= 40 视为异常（丢线/弯道突变），停止累加并计入 s_vision_ctx.lost_count
...
static void find_mid_line_weight(void)//图像中值 与 补线中值差 然后 计算平均偏差数
{
    ...
    for(uint8_t i = 15 ; i < VISION_ZIP_IMAGE_H - 1 ; i++)
    {
        ...
        {
            weight_mid_line_sum += (uint16_t)s_vision_ctx.mid_line_per_row[i] * mid_weight_list[i];  // 修正：加权中线，而不是偏差
