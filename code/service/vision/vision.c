...
static uint8_t mt9v03x_image_bandw_zip[60][94] = {{0}};


// 边线与中线数据，按行存储（索引 0=图像顶部，59=图像底部）
static uint8_t left_line_list[zip_MT9V03X_H] = {0} ; // 每行的左边界列坐标
static uint8_t right_line_list[zip_MT9V03X_H] = {0}; // 每行的右边界列坐标
static uint8_t mid_line_list[zip_MT9V03X_H] = {0};   // 每行的中线列坐标（左右边界均值）
...
void Vision_GetDebugSnapshot(vision_debug_snapshot_t *p_snapshot)
{
    ...
    p_snapshot->image_width = zip_MT9V03X_W;
    p_snapshot->image_height = zip_MT9V03X_H;
    p_snapshot->line_count = zip_MT9V03X_H;
    p_snapshot->calculate_error = calculate_error;
}
