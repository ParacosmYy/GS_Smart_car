 ...
 *
 * @par Processing flow
 * Raw grayscale frame → OTSU threshold → downsampled binary image
...
static uint32_t s_otsu_pixel_count[OTSU_GRAY_LEVELS];
...
/**
 * @brief 从原始灰度帧直接生成压缩二值图。
 *
 * Steps:
 *   1. 隔行隔列采样原始灰度帧。
 *   2. 按 OTSU 阈值直接写入压缩二值图。
 *
 * @param[in] threshold 二值化阈值。
 * @return void。
 */
static void Vision_BuildBinaryZip(uint8_t threshold)
{
    uint8_t zip_row = 0U;
    uint8_t (*p_camera_image)[VISION_RAW_W] = (uint8_t(*)[VISION_RAW_W])SmartcarHal_CameraData();

    for (uint8_t raw_row = 0U; raw_row < VISION_RAW_H; raw_row += 2U)
    {
        uint8_t zip_col = 0U;

        for (uint8_t raw_col = 0U; raw_col < VISION_RAW_W; raw_col += 2U)
        {
            if (p_camera_image[raw_row][raw_col] < threshold)
            {
                s_vision_ctx.binary_zip[zip_row][zip_col] = VISION_BINARY_BLACK;
            }
            else
            {
                s_vision_ctx.binary_zip[zip_row][zip_col] = VISION_BINARY_WHITE;
            }
...
static uint8_t otsu(const uint8_t *p_image, uint16_t width, uint16_t height)
{
    ...
    for (i = 0; i < height; i++)
    {
        for (j = 0; j < width; j++)
        {
            s_otsu_pixel_count[p_image[((uint32_t)i * (uint32_t)width) + (uint32_t)j]]++;
        }
    }
    ...
    for (i = 0; i < OTSU_GRAY_LEVELS; i++)
    {
        sum += (float)i * (float)s_otsu_pixel_count[i];
    }
    ...
    for (i = 0; i < OTSU_GRAY_LEVELS; i++)
    {
        w_b += s_otsu_pixel_count[i]; // 前景像素数
        if (w_b == 0U)
        {
            continue;
        }
        ...
        sum_b += (float)i * (float)s_otsu_pixel_count[i];
 ...
 *   2. 按阈值直接生成压缩二值图。
 *   3. 二值图像滤波（去孤立噪点）
 *   4. 边线检测与中线计算（写入 left/right/s_vision_ctx.mid_line_per_row）
 *   5. 加权中线 + 丢线检测（输出 s_vision_ctx.calculate_error、s_vision_ctx.lost_count）
 *
...
void Vision_Process(void)
{
    ...
    // 步骤 2：按阈值直接生成压缩二值图
    Vision_BuildBinaryZip(s_vision_ctx.image_threshold);
    // 步骤 3：4 邻域滤波去噪
    Bin_Image_Filter();
    // 步骤 4：从底向顶逐行搜索左右边线，计算每行中线
    find_mid_line();
    // 步骤 5：行加权平均得到最终中线 s_vision_ctx.mid_line 与偏差 s_vision_ctx.calculate_error
