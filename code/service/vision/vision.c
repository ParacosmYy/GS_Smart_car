 ...
 *      Author: Paracosm
 *
 * @brief 视觉处理模块：灰度图像 → OTSU 二值化 → 压缩 → 滤波
 *        → 边线检测 → 加权中线 → 偏差信号（PID 控制器输入）
 *        摄像头原始分辨率 188×120（MT9V03X_H × MT9V03X_W）
 *        压缩后 94×60（zip_MT9V03X_W × zip_MT9V03X_H）
 */
...
uint8_t  mt9v03x_image_bandw[MT9V03X_H][MT9V03X_W] = {{0}};
 ...
 *   7. 调试画线（draw_all_lines_test，显示到屏幕）
 *
 * @note 调用后控制器读取 calculate_error（PID 输入）和 lost_count（停止判断）
 */
void Vision_Process(void)
{
    // 步骤 1：OTSU 自适应阈值，传入原始灰度图首地址（188×120 = MT9V03X_W × MT9V03X_H）
